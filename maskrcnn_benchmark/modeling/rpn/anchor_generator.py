...
def make_anchor_generator_retinanet(config):
    anchor_sizes = config.MODEL.RETINANET.ANCHOR_SIZES     # different area sizes
    aspect_ratios = config.MODEL.RETINANET.ASPECT_RATIOS   # equal area and different length and width
    anchor_strides = config.MODEL.RETINANET.ANCHOR_STRIDES # 
    straddle_thresh = config.MODEL.RETINANET.STRADDLE_THRESH
    octave = config.MODEL.RETINANET.OCTAVE                 # it is base number
    scales_per_octave = config.MODEL.RETINANET.SCALES_PER_OCTAVE # based on OCTAVE
